...
signed main(signed argc, char **argv)
{
  ...
  if (!argc) { printf("usage: c4 [-s] [-d] file ...\n"); return -1; }

  if ((fd = open(*argv, 0)) < 0) { printf("could not open(%s)\n", *argv); return -1; }

  poolsz = 256*1024; // arbitrary size
  if (!(sym = (int*)(intptr_t)malloc(poolsz))) { printf("could not malloc(%lld) symbol area\n", poolsz); return -1; }
  if (!(le = e = (int*)(intptr_t)malloc(poolsz))) { printf("could not malloc(%lld) text area\n", poolsz); return -1; }
  if (!(data = (char*)(intptr_t)malloc(poolsz))) { printf("could not malloc(%lld) data area\n", poolsz); return -1; }
  if (!(sp = (int*)(intptr_t)malloc(poolsz))) { printf("could not malloc(%lld) stack area\n", poolsz); return -1; }
  ...
  if (!(lp = p = (char*)(intptr_t)malloc(poolsz))) { printf("could not malloc(%lld) source area\n", poolsz); return -1; }
